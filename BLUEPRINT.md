...
換即時稿引擎的候選一律跑真實會議錄音，不跑 `near.wav`。後者是立法院質詢、全中文，量不出雙語能力；實際會議滿是英文產品名。近場測試贏、真實會議輸的情況已經發生過一次。
...
- 聲紋分不出是誰的遠端片段掛在 `remote` 這個識別碼下。它代表「有人在講，但不確定是誰」，底下可能有好幾位不同的人，所以三條規則同時成立：顯示成「遠端」、不佔用「語者 N」的編號、不接受命名。畫面、HTML 匯出與摘要 prompt 必須說出同一個名字；任何一條路徑自己算一次，同一位語者就會在不同地方變成不同的人。
- 「我是王小明」可建立 `proposed_name = 王小明`，狀態為待確認。
- 使用者確認後才建立 `confirmed_name`。
- 「王小明剛才說……」不得用來認定目前發言者就是王小明。
- 語者合併或拆分保留稽核紀錄，引用仍指向原始片段。
- `stt::speakers` 的 `judge` 對夠長的片段採「不夠像就是別人」：錯拆看得見、錯併看不見，這個取捨是刻意的。代價是口罩、距離、感冒都可能讓同一個人多出一列。
- 合併是**讀取時套用的別名**，不是對過去的改寫。`SpeakerMerged` 只在來源那一列寫下 `merged_into`：片段的 `speaker_id` 不動，被併掉的那一列也留在名單裡。每條讀取路徑（畫面、匯出、摘要 prompt）各自依自己的游標把 id 收斂到合併後的那一位，所以早於合併的快照讀出來仍是合併之前的樣子，一份已經匯出的紀錄不會因為之後的操作而改口。錄音分頁的語者列上有「併入」，`stt::speakers` 那個「寧可錯拆也不錯併」的取捨因此改得掉。`SpeakerSplit` 仍然沒有生產者。

### 8.2 名稱優先順序

```text
使用者確認名稱 > 使用者手動名稱 > 待確認自我介紹 > 語者 N > 遠端（辨識不出的遠端發言）
```
...
## 17.4 第三輪獨立審查（v0.1.0 發布之後）

`v0.1.0` 送出去之後才跑的一輪跨模型唯讀審查，十八項全部處理。這一輪與前兩輪
不同的地方在於它是對著**已經發布的產物**做的，因此有四項是「使用者現在就踩得
到」而不是「將來可能會踩到」。

貫穿這一輪的教訓有三條，比個別修法重要：

1. **「回報成功」與「真的開始了」是兩件事。** 任何 `start()` 在真正的初始化
   之前回傳，失敗就變成「畫面說在錄、實際什麼都沒有」。三個地方同時中了。
2. **列數契約擋不住「動到了別人那一列」。** 防線問的是有沒有動到一列，不是
   有沒有動到對的那一列。全域 id 只能有一個配發者。
3. **只查範圍卻不拿來比對的欄位等於沒有被驗證。** locator 曾經只被檢查在不在
   範圍內，引文則對整段內容比對 —— 那個數字沒有任何人查。

| 症狀 | 根因 | 教訓 |
|---|---|---|
| 捏造的 Fact 拿到 `valid` | `locator: "0-0"` 過得了「終點不超過長度」，而引文是拿去跟整段內容比對的，只要片段裡有一個句號就成立 | 空引文修掉之後，同一個 bug class 還剩兩個成員：零寬區間與無資訊量的引文。修症狀不等於修病灶 |
| 一場會議的成果覆寫另一場的 | `document_id`／`run_id` 有兩個配發者（Session 快取 `MAX(id)+1`、歷史頁每次重讀），撞號被 `ON CONFLICT DO NOTHING` 吞掉 | 配發與寫入之間隔多久都不能被別人拿走，所以遞增與讀取要在同一個 statement 裡 |
| 雙擊「開始錄音」讓進行中的會議不再產生逐字稿 | 四個步驟只有最後一步在 Session 的鎖裡，第二次呼叫走完前三步才被拒絕，而第三步已經換掉了錄音來源 | 拒絕得太晚等於沒有拒絕。要嘛整段互斥，要嘛第一步就檢查 |
| macOS 權限被拒仍顯示「錄音中、狀態正常」 | `MacCapture::start` 只 spawn 執行緒就回 `Ok`，開裝置在之後 | 見上面第 1 條。做法是握手：背景執行緒先回報初始化結果 |
| 48 kHz 的系統音訊被當成 16 kHz | 只檢查位元組數是四的倍數，格式從設定推而不是從交付的資料讀 | 設定是請求不是保證。時間軸錯三倍，而聲音只是聽起來快一點 —— 這種錯不會自己浮出來 |
| 立體聲變成「左聲道播完再播右聲道」 | 把 `AudioBufferList` 的所有 buffer 首尾相接 | 交錯與分平面的差別從位元組長度看不出來，要看 `number_channels` |
| 模型檔壞掉，錄完整場才發現一個字都沒有 | 引擎在工作執行緒裡載入，失敗只留一行 log 就 return，而定稿執行緒死掉會讓分流停止讀音訊 | 見上面第 1 條 |
| macOS 版找不到模型，`stt.log` 寫不進去 | 路徑全部相對於 `current_exe().parent()`，而那在 `.app` 裡是 `Contents/MacOS/` | 「執行檔旁邊」在三個平台是三個意思。日誌要放一定寫得進去的地方，否則它只在不需要它的時候有效 |
| Windows 的 MSI 與 NSIS 裝出來進不了 `main` | DLL 是 `tauri build` 之後才複製到外層 portable 目錄的，安裝檔內部沒有宣告任何 resources | 沒有人走過的那條路徑不會因為沒人抱怨就是對的。要驗就把安裝檔攤開來看，不要在它的位元組裡找檔名 |
| 長會議吃掉全部記憶體 | 擷取到兩個引擎之間全是無界 channel | 有界加上記錄丟棄量。無界佇列是把「現在很快」當成不變量 |
| 每次停止都少掉最後一個字的韻尾 | 不足一批的 PCM 與重取樣器的 carry 都直接丟棄 | 收尾要 flush，而補零湊出來的那一段不能推進時間軸 |
| 逐字稿可以繞過 Prompt 圍欄 | 圍欄只包住本輪證據；上一版成果與 schema 重試的區塊都由逐字稿長出來，卻放在圍欄外面 | 圍欄要包住「所有不受信任的東西」，不是「叫做證據的那一段」 |
| 一輪生成可以打三百次模型 | 重試上限是每個區塊兩次，`max_rounds` 管的是迴圈圈數 | 費用上限必須是呼叫次數的上限 |
| 生成永遠停在「生成中」 | 180 秒計時器在 stdin 寫完之後才起跑，而寫入本身會在 CLI 不讀時永久阻塞 | 計時器要涵蓋它要保護的那段時間 |
| 逾時之後 CLI 的子孫行程繼續燒額度 | `taskkill /T` 只有 Windows 那一份 | 跨平台的收尾要每個平台各寫一次，`#[cfg]` 少一個就是少一個平台 |
| 游標為 10 的 Prompt 裡出現 seq 11 才確認的名字 | 查詢只用 `created_event_seq` 界定，名字卻讀投影上現在的值 | 加一個 `confirmed_event_seq` 欄位也擋不住：同一位語者可以被改名兩次，投影只留得下最後一次。「那一刻他叫什麼」只有日誌答得出來 |
| 重建投影把被中斷的會議變回「錄音中」 | 改名與 `failed` 都是就地 UPDATE，日誌裡沒有它們 | 「所有投影可重建」只要有一個欄位是就地 UPDATE 的就不成立，而它會安靜地存在到有人真的跑重建那一天 |
| 畫面寫 Alice、匯出檔寫 `s1` | `RenderContext` 沒有語者名稱映射 | 修了畫面那一側不等於修完。兩個渲染器顯示同一位語者時必須說出同一個名字 |
| 畫面沒有目錄、決議少一層小標 | 兩個渲染器各自演化 | 與 §17.3 完全同一條教訓，隔了一輪又發生一次 |

審查明確沒找到問題的項目：`meeting.ts` 的使用者編輯優先權、零區塊不得記為成功、
`revise_of` 只指向已完成版本、引用的身分／revision／快照游標／SHA-256 基本檢查、
repo 自有的 macOS `unsafe`、密鑰儲存與輸出路徑、以及 macOS 靜態／Windows 動態
連結的策略本身。
