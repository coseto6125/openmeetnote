...
- **`SpeakerMerged` 與 `SpeakerSplit` 目前沒有生產者，所以上面那個代價還沒有修法。**Store 有事件、投影與 `merged_into` 欄位，但它是就地改寫 `speaker_id`，與本節「引用仍指向原始片段」相牴觸，而且會回溯改寫早於合併的快照。接上使用者操作之前要先改成保留原始 id、讀取時依游標套用 alias。
