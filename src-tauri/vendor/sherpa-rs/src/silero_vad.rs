...
impl SileroVad {
    ...
    ///
    /// `clear` only drains the segment queue; the model keeps the recurrent
    /// state from whatever it has already seen. Reusing one detector to judge
    /// independent buffers without this leaks the previous buffer's speech
    /// into the next verdict (measured: noise judged as 1100 ms of speech
    /// right after a speech buffer, 0 ms on its own).
