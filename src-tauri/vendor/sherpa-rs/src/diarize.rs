...
impl Diarize {
    ...
    pub fn compute(
        ...
        unsafe {
            ...
            // 上游少了這個檢查：處理失敗時 C 端回傳 null，直接拿去
            // GetNumSegments 會存取違規（Windows 上表現為 0xc0000005）。
            if result.is_null() {
                bail!("Diarization returned no result.");
            }
            ...
            if !segments_ptr.is_null() && num_segments > 0 {
                ...
                // 上游在這裡直接 bail，跳過了下面兩行釋放。分不出語者的批次
                // 會整個洩漏 result，累積幾次之後堆就壞了 —— 表現為「跑一陣子
                // 才崩」，而不是每次都崩，因此特別難追。
